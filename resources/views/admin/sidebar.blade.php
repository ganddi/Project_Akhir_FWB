  <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">

            <!-- Sidebar - Brand -->
            <a class="sidebar-brand d-flex align-items-center justify-content-center" href="index.html">
                <div class=" mx-4">mudahBerkemah</div>
            </a>

            <!-- Divider -->
            <hr class="sidebar-divider my-0">

            <!-- Nav Item - Dashboard -->
            <li class="nav-item active">
                <a class="nav-link" href="index.html">
                    <i class="fas fa-fw fa-tachometer-alt"></i>
                    <span>Dashboard</span></a>
            </li>

            <!-- Divider -->
            <hr class="sidebar-divider">

            <!-- Heading -->
            <div class="sidebar-heading">
                Item Manager
            </div>

            <!-- Nav Item - Lihat Item -->
            <li class="nav-item">
                <a class="nav-link" href="/lihatItem">
                    <i class="fas fa-fw fa fa-th-list"></i>
                    <span>Lihat Item</span></a>
            </li>

            <li class="nav-item">
                <a class="nav-link" href="/tambahItem">
                    <i class="fas fa-fw fa fa-plus"></i>
                    <span>Tambah Item</span></a>
            </li>

           

            <!-- Divider -->
            <hr class="sidebar-divider">

            <!-- Heading -->
            <div class="sidebar-heading">
                Rental Manager
            </div>

            <!-- Nav Item - Pages Collapse Menu -->
            <li class="nav-item">
                <a class="nav-link" href="/belum_dibayar" >
                    <i class="fas fa-fw fa fa-shopping-cart"></i>
                    <span>Belum Dibayar</span></a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="/diPinjam" >
                    <i class="fas fa-fw fa fa-handshake"></i>
                    <span>Dipinjam</span></a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="/diKembalikan" >
                    <i class="fas fa-fw fa fa-history"></i>
                    <span>Dikembalikan</span></a>
            </li>

            <!-- Divider -->
            <hr class="sidebar-divider d-none d-md-block">

            <!-- Heading -->
            <div class="sidebar-heading">
                User Manager
            </div>

            <!-- Nav Item - Pages Collapse Menu -->
            <li class="nav-item">
                <a class="nav-link" href="/lihatUser" >
                    <i class="fas fa-fw fa fa-user"></i>
                    <span>Lihat User</span></a>
            </li>

            <!-- Divider -->
            <hr class="sidebar-divider d-none d-md-block">

            {{-- <!-- Sidebar Toggler (Sidebar) -->
            <div class="text-center d-none d-md-inline">
                <button class="rounded-circle border-0" id="sidebarToggle"></button>
            </div> --}}

        </ul>